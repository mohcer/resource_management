...
class UserDto:
    ...
    user_res = api.model(
        "user_res",
        {
            ...
            "email": fields.String(required=True, description="unique user email"),
            "user_registered_on": fields.DateTime(
                required=True, description="user registered on"
            ),
            "user_quota": fields.Integer(required=True, description="user quota"),
            "quota_remaining": fields.Integer(
                required=True, description="user quota remaining"
            ),
        },
    )

    user_res_model = api.model(
        "user_res_details",
        {
...
class ResourceDto:
    ...
    api = Namespace(
        "resource", description="request object to create platform User Resources"
    )

    resource_req_data = api.model(
        "resource_req_data",
        {"resource_name": fields.String(required=True, description="Resource name")},
    )

    resource_res = api.model(
        "resource_res",
        {
            "user_id": fields.Integer(required=True, description="user id"),
